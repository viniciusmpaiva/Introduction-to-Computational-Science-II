...
char* concatOverlap(char* a, char* b, int overlap,char s_overlap_string[]){
    int a_size = strlen(a);
...
char* recOverlap(char** reads, int N,char s_overlap_string[]){
    ...
    for(int i=0; i<N; i++){
        if( i!=0){
            for(int u= 0; u<i; u++){
                ...
                if(overlap>=b_overlap && overlap !=first_overlap){
                    ...
                    strcpy(s_overlap_string,concatOverlap(reads[i],reads[u],b_overlap,s_overlap_string));
        ...
        for(int j=i+1; j<N; j++){
            ...
            if(overlap>=b_overlap&&overlap!=first_overlap){
                b_overlap = overlap;
                first_overlap = overlap;
                strcpy(s_overlap_string,concatOverlap(reads[i],reads[j],b_overlap,s_overlap_string));
                final_i = i;
                final_j = j;
            }
        }
    ...
    return recOverlap(newreads,N,s_overlap_string); 
}
